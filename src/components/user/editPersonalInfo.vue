...
export default {
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      this.newAvatarUrl = res.data   // 结果返回的头像url
      this.myInfoModel.avatarUrl = URL.createObjectURL(file.raw) // URL对象表示指定的 File 对象或 Blob 对象
    },
    // 上传前校验
    ...
    doSaveMyInfo() {
      ...
      .then(response => {
        ...
        this.$store.dispatch('setAvatarUrl', this.newAvatarUrl) // 修改全局变量
        this.loadingVisible = false
    ...
    this.$nextTick(() => {  
      this.$on('openDialog', function(action) {
        ...
        .then(response => {
          ...
          this.myInfoModel = responseData
          this.myInfoModel.username = responseData.userName
